...
export class Deployable {
    ...
    static hasFactions(deploy: T.Deployable): boolean {
        return !!deploy.factions && deploy.factions.some(f => Object.values(f).some(v => !!v));
    }
...
export class GachaPool {
    ...
    static isNormal(pool: T.GachaPool): boolean {
        return !this.isJointOperation(pool) && ['NORMAL', 'DOUBLE', 'CLASSIC', 'CLASSIC_DOUBLE', 'LINKAGE', 'LIMITED', 'SINGLE'].includes(pool.client.gachaRuleType as string);
    }
}
